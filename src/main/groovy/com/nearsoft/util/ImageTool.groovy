...
class ImageTool {
    ...
    public void loadMask(String file) {
        ...
        if (!mask) {
            mask = JAI.create("stream", getByteStream(file));
            masks[file] = mask
        }
    }
    ...
    public void loadAlpha(String file) {
        ...
        if (!alpha) {
            alpha = JAI.create("stream", getByteStream(file));
            alphas[file] = alpha;
        }
    }
    ...
    public void load(byte[] bytes) {
        image = JAI.create("stream", getByteStream(bytes));
    }
    ...
    /**
     * Returns the original image as a byte array.
     *
     * @param type file type for the image
     * @see <a href="http://java.sun.com/products/java-media/jai/iio.html">Possible JAI encodings</a>
     */
    public byte[] getOriginalBytes(String type) throws IOException {
        if(!original){
            throw new IOException('You must to save the original before try to use it.')
        }
        getBytes(type, ORIGINAL)
    }
     ...
     * @see <a href="http://java.sun.com/products/java-media/jai/iio.html">Possible JAI encodings</a>
     */
    public byte[] getResultBytes(String type) throws IOException {
        if(!reuslt){
            throw new IOException('The image must be processed before to get a result.')
        }
        getBytes(type, RESULT)
    }
    ...
    /**
     * Read a file and return a ByteArraySeekableStream
     * @param file the location of the file.
     * @return ByteArraySeekableStream object
     */
    private static ByteArraySeekableStream getByteStream(String file){
        File fileToLoad = new File(file)
        if(!fileToLoad.exists() || !fileToLoad.isFile()){
            throw new IOException("The file doesn't exists")
        }
        getByteStream(fileToLoad.bytes)
    }
     ...
     * @return ByteArraySeekableStream object
     */
    private static ByteArraySeekableStream getByteStream(byte[] bytes){
        new ByteArraySeekableStream(bytes);
    }

    /**
     * Get the bytes with the required TYPE which can be JPEG or PNG
     * @param type
     * @param option
     * @return
     */
    private byte[] getBytes(String type, String option){
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        JAI.create(ENCODE, ORIGINAL.equals(option) ?  original : result, bos, type, null);
        bos.toByteArray()
    }
}
